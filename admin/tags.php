    ...
    if (empty($tag_name)) {
        ...
        try {
            ...
            if ($check->rowCount() > 0) {
                ...
                $success = "Etiket başarıyla eklendi.";
            }
...
// 3. ETİKETLERİ LİSTELEME
$tags = $db->query("SELECT * FROM tags ORDER BY id DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="tr">
<head>
    <meta charset="UTF-8">
    <title>Etiket Yönetimi - CMS</title>
    <style>
        ...
        form button { background: #3498db; color: white; border: 0; padding: 10px 20px; border-radius: 4px; cursor: pointer; font-size: 16px; }
...
<div class="main-content">
    ...
    <p>Yazılarınıza birden fazla eklenebilecek etiketleri (anahtar kelimeleri) buradan yönetin.</p>
    ...
    <div class="container">
        <div class="box form-box">
            ...
            <form action="tags.php" method="POST">
                ...
                <input type="text" name="tag_name" placeholder="Örn: php8, mysql, backend" required>
                <button type="submit" name="add_tag">Kaydet</button>
            </form>
        </div>
                    ...
                    <?php if(count($tags) > 0): ?>
                        <?php foreach($tags as $tag): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="tags.php?delete=<?php echo $tag['id']; ?>" class="btn-delete" onclick="return confirm('Bu etiketi silmek istediğinize emin misiniz?')">Sil</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="4" style="text-align: center; color: #999;">Henüz hiç etiket eklenmemiş.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
